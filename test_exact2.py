...
def get_video_exact(share_url):
    ...
    # 3. Find the URL for this ref_id in ALL of HTML (it might not be in __next_f!)
    # Actually, if we just find `"REF_ID":"https...` or `REF_ID:"https...` or `REF_ID:["https...`
    # Let's search the whole HTML for `ref_id` followed by `https://...mp4`
    
    pattern = r'\"?' + ref_id + r'\"?(?:\\?\":|:\[?)\\?\"(https://[^\"]+\.mp4[^\"]*)'
